...
export default function GeoRules(){
  ...
  const load = async () => { try { const r = await axios.get('/api/v1/georules'); setRules(r.data || []) } catch (e) { console.error(e) } }
  React.useEffect(()=>{ load() }, [])

  const create = async () => {
    try { await axios.post('/api/v1/georules', { zone_id: zone, match_type: matchType, match_value: matchValue, target }); setZone(''); load() } catch (e) { alert('Error creating rule') }
  }

  const testResolve = async () => {
    try { if (!zone) { alert('Select a zone'); return } const r = await axios.post('/api/v1/georules/resolve', { zone_id: zone, client_ip: testIp }); setResolveResult(r.data) } catch (e) { alert('Error resolving') }
  }
  ...
  return (
    <div className="bg-white shadow rounded p-6">
      ...
      <div className="grid gap-2 mb-4 max-w-md">
        <input className="border rounded px-3 py-2" placeholder="zone id" value={zone} onChange={e=>setZone(e.target.value)} />
      ...
      <table className="w-full table-auto border-collapse">
        <thead>
          <tr className="bg-gray-50"><th className="border px-2 py-1">ID</th><th className="border px-2 py-1">Zone</th><th className="border px-2 py-1">Type</th><th className="border px-2 py-1">Value</th><th className="border px-2 py-1">Target</th></tr>
        </thead>
        <tbody>
          {rules.map(r => (
            <tr key={r.id}><td className="border px-2 py-1">{r.id}</td><td className="border px-2 py-1">{r.zone_id}</td><td className="border px-2 py-1">{r.match_type}</td><td className="border px-2 py-1">{r.match_value}</td><td className="border px-2 py-1">{r.target}</td></tr>
          ))}
        </tbody>
      </table>
    </div>
