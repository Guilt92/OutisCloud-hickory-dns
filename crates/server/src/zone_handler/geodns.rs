...
use crate::proto::rr::{Name, LowerName, RData, RecordSet, RecordType, TSigResponseContext};
...
mod tests {
    ...
    fn test_geo_handler_skip_non_a() {
        ...
        let query = crate::proto::rr::LowerQuery::new(&Name::from_str("www.example.com.").unwrap(), RecordType::NS, 1);
        let result = futures_util::executor::block_on(handler.lookup(&query.name(), RecordType::NS, Some(&info), LookupOptions::default()));
    ...
    fn test_geo_handler_basic() {
        ...
        let info = make_reqinfo("1.2.3.4");
        let query = crate::proto::rr::LowerQuery::new(&Name::from_str("www.example.com.").unwrap(), RecordType::A, 1);
        // no match since geo DB not available
        let result = futures_util::executor::block_on(handler.lookup(&query.name(), RecordType::A, Some(&info), LookupOptions::default()));
        assert!(matches!(result, LookupControlFlow::Skip));
    }
    ...
    fn test_geo_handler_match() {
        ...
        let query = crate::proto::rr::LowerQuery::new(&Name::from_str("www.example.com.").unwrap(), RecordType::A, 1);
        match futures_util::executor::block_on(default_handler.lookup(&query.name(), RecordType::A, Some(&info), LookupOptions::default())) {
